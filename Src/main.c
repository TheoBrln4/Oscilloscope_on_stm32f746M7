...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  int quotient = globvar2/10;
	  int reste = globvar2%10;
	 /** LED_DispGreen(0);
	  LED_DispGreen(1);
	  LED_DispGreen(0); **/
	  sprintf(bufStr, "time:%d.%d s", quotient, reste);
	  BSP_LCD_DisplayStringAt(20,80,(uint8_t *) bufStr,LEFT_MODE);

	  sprintf(bufStr, "frequency:%d Hz", frq);
	  BSP_LCD_DisplayStringAt(20,110,(uint8_t *) bufStr,LEFT_MODE);

	  BSP_LCD_DrawPixel(t, sortieADC*200/4096, 0);
	  if(t>= 479){
		  BSP_LCD_Clear(LCD_COLOR_WHITE);
	  }
	  LED_DispGreen(globvar);
    /* USER CODE END WHILE */
...
static void MX_TIM5_Init(void)
{
  ...
  TIM5->DIER = TIM5->DIER|0x1;
  TIM5->CR1 = TIM5->CR1|0x1;
...
static void MX_TIM7_Init(void)
{
  ...
  TIM7->DIER = TIM7->DIER|0x1;
  TIM7->CR1 = TIM7->CR1|0x1;
